...
const convert = (dictdir, dictID, destPath) => {
  ...
  _.forEach(dict.data, (entry) => {
    ...
    node.ele('h1').txt(cleanXml(entry.index));
    const body = node.ele('div');
    _.forEach(entry.dat, (dt) => _.forEach(dt.text.split('\n'), (par) => body.ele('p').txt(cleanXml(par))));
    bar.tick(1);
  });
  console.log('Saving...');
  fs.writeFileSync(path.join(destDir, 'Dictionary.xml'), nd.end());
