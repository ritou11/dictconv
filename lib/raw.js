...
const fromIfo = (ifoFile, destPath) => {
  ...
  try {
    const unzipResult = spawnSync('gunzip', ['-f', '-S', '.dz', `${barename}.dict.dz`], {
      stdio: 'inherit',
    });
  
    if (unzipResult.status !== 0) {
      console.error('Error unzipping file:', unzipResult.stderr?.toString() || 'Unknown error');
      return undefined;
    }
  } catch (e) {
    console.error(e);
...
const fromArchive = (filename, destPath) => {
  ...
  const result = spawnSync('tar', ['-xjf', filename, '-C', tmpdir], {
    stdio: 'inherit',
  });

  if (result.status !== 0) {
    console.error('Error extracting archive:', result.stderr?.toString() || 'Unknown error');
    return undefined;
  }
